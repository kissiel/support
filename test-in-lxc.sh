...
start_lxc_for(){
    ...
    if ! sudo $LXC_LS |grep -q $pristine_container; then
        ...
        echo $step
        if ! /usr/bin/time -o $TIMING sudo $LXC_CREATE  -n $pristine_container -t ubuntu -- -r $target --user=$CONTAINER_USER --packages=software-properties-common,python3-dev >$LOG_DIR/$target.pristine.log 2>$LOG_DIR/$target.pristine.err; then
            outcome=1
    ...
    echo $step
    if ! /usr/bin/time -o $TIMING sudo -E $LXC_ATTACH --keep-env -n $target_container >$LOG_DIR/$target.provision.log 2>$LOG_DIR/$target.provision.err -- bash -c "$PWD/support/provision-testing-environment $PWD"; then
...
fix_permissions(){
    ...
    if ! sudo -E $LXC_ATTACH --keep-env -n $target_container -- bash -c "chown -R --reference=$PWD/support/test-in-lxc.sh $PWD" >$LOG_DIR/$target.fix-perms.log 2>$LOG_DIR/$target.fix-perms.err; then
